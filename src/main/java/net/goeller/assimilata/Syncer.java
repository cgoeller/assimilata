...
public class Syncer {
	...
	public SyncJob prepare(final SyncSet syncSet) {
		...
		SyncJob synchJob = new SyncJob(syncSet);

		if (syncSet.hasOption(Option.COPY_TO_TARGET)) {
			Visitor visitor = new Visitor(new SourceRunDelegate(syncSet, synchJob, stats));
			try {
		...
		if (syncSet.hasOption(Option.DELETE_FROM_TARGET)) {
			Visitor visitor = new Visitor(new TargetRunDelegate(syncSet, synchJob, stats));
			try {
	...
	public void sync(final SyncJob syncJob) {
		if (syncJob.getSyncSet().isDryRun()) {
			log.info("Dry run: Skipping synch phase");
			return;
		}
		...
		try {
			for (Entry entry : syncJob.getSynchEntries()) {
				entry.execute();
			}
		} catch (IOException e) {
